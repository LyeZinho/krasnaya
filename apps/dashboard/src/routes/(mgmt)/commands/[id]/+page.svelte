<script lang="ts">
    ...
    async function saveCommand() {
        ...
        try {
            ...
            navigationTimeout = setTimeout(() => goto("/commands"), 1500);
        } catch (err) {
    ...
    async function deleteCommand() {
        try {
            ...
            navigationTimeout = setTimeout(() => goto("/commands"), 1000);
        } catch (err) {
...
<div class="space-y-8" role="region" aria-label="Command editor" onkeydown={handleKeydown}>
    ...
    {#if mode === "visual"}
        <div class="space-y-6">
            ...
            <BrutalCard title="Lógica do Comando">
                <textarea
                    value={command.logic ? JSON.stringify(command.logic, null, 2) : '{}'}
                    onchange={(e) => {
                        try {
