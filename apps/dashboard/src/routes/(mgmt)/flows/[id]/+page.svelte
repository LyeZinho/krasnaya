<script lang="ts">
    ...
    let jsonData = JSON.stringify(automation, null, 2);
    ...
    async function saveAutomation() {
        ...
        try {
            ...
            navigationTimeout = setTimeout(() => goto("/flows"), 1500);
        } catch (err) {
    ...
    async function deleteAutomation() {
        try {
            ...
            navigationTimeout = setTimeout(() => goto("/flows"), 1000);
        } catch (err) {
...
<div class="space-y-8" role="region" aria-label="Flow editor" onkeydown={handleKeydown}>
